...
document.addEventListener('change', function (event) {
  if (event.target.classList.contains('form-select-categoria')) {
    ...
    const fila = event.target.closest('tr'); // Obtener la fila correspondiente
    const tipoSelect = fila.querySelector('.form-select-tipo'); // Campo de Tipos

    // Limpiar las opciones del campo de Tipos
    tipoSelect.innerHTML = '<option selected disabled>Seleccionar</option>';

    // Añadir las opciones relacionadas según la categoría
...
// Función para validar la fecha de entrega y vencimiento
function validarFila(row) {
  const categoria = row.querySelector('.form-select-categoria').value;
  const tipo = row.querySelector('.form-select-tipo').value;
  const cantidad = row.querySelector('.form-control-cantidad').value;

  if (!categoria || !tipo || cantidad <= 0) {
