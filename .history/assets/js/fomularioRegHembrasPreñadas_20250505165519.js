function imprimirFormulario() {
  ...
  const printableContent = document.getElementById('printableSection').cloneNode(true);
  
  // **Eliminar cualquier título duplicado**
  const extraTitles = printableContent.querySelectorAll("h2");
  if (extraTitles.length > 1) {
    extraTitles[0].remove();  // Borra el título adicional si existe
  }

  // **Ocultar el formulario antes de imprimir**
  document.getElementById('cerdaForm').style.display = "none";

  // **Insertar la versión corregida y ejecutar impresión**
  document.body.appendChild(printableContent);
  window.print();
  document.body.removeChild(printableContent);

  // **Restaurar la visibilidad**
  document.getElementById('cerdaForm').style.display = "block";
