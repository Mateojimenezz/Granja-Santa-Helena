...
function filtrarTabla() {
  const filtro = document.getElementById('busqueda').value.toLowerCase().trim(); // Convertir texto a minúsculas y eliminar espacios innecesarios
  const filas = document.querySelectorAll('#userTable tbody tr'); // Seleccionar filas

  let filasVisibles = 0; // Contador de filas visibles para depuración

  filas.forEach(fila => {
    const textoFila = fila.textContent.toLowerCase().trim(); // Convertir texto de la fila
    if (textoFila.includes(filtro)) {
      fila.style.display = ''; // Mostrar la fila
      filasVisibles++; // Incrementar contador
    } else {
      fila.style.display = 'none'; // Ocultar la fila
    }
  });

  // Depuración: Mostrar un mensaje si no hay filas visibles
  if (filasVisibles === 0) {
    console.log(`No se encontraron coincidencias para: "${filtro}"`);
  }
}

// Activar la función al escribir en la barra de búsqueda
document.getElementById('busqueda').addEventListener('input', filtrarTabla);

// Activar la función al hacer clic en el botón de búsqueda
