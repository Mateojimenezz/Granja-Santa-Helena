  ...
  function exportarExcel() {
    ...
    tablaTemporal.querySelectorAll('tbody tr').forEach(fila => {
      ...
      columnas.forEach(celda => {
        ...
        const input = celda.querySelector('input'); // Detectar <input>
        ...
        if (select) {
          ...
          console.log('Texto seleccionado:', textoSeleccionado); // Depuración
          ...
          if (input.type === 'checkbox') {
            ...
            // Capturar y ajustar la fecha al formato local
            const fecha = new Date(input.value + 'T00:00:00');
            celda.textContent = fecha.toISOString().split('T')[0]; // Convertir a YYYY-MM-DD
          } else {
            // Capturar valores de otros inputs
            celda.textContent = input.value || 'N/A';
          }
        }
      });
    });
