...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const items = document.querySelectorAll("#sisidebarMenu .nav-link");
const icons = [
  "bi bi-people", // Usuarios
  "bi bi-box-seam", // Inventarios
  "bi bi-pencil-square", // Registro de Producción
  "bi bi-heart-pulse", // Salud Animal
  "bi bi-cash", // Ventas
];

items.forEach((item, index) => {
  const icon = document.createElement("i");
  icon.className = `icon-left ${icons[index]}`;
  item.prepend(icon); // Añade el ícono antes del texto
});
