...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const iframeContent = document.getElementById("iframe-content"); // Iframe para cambiar su tamaño
  ...
  // Ajustar el contenedor principal e iframe dinámicamente
  function adjustMainContent() {
    if (sidebar.style.display === "none") {
      mainContent.style.margin = "0px"; // Elimina el margen izquierdo
      mainContent.style.width = "100vw"; // Ancho completo
      if (iframeContent) {
        iframeContent.style.width = "100vw"; // Ajusta el iframe

      }
    } else {
      mainContent.style.marginLeft = "250px"; // Ajusta el margen izquierdo
      mainContent.style.width = `calc(100% - ${sidebar.offsetWidth}px)`; // Calcula el ancho dinámico
      if (iframeContent) {
        iframeContent.style.width = "100%"; // Asegura que el iframe ocupe el ancho de mainContent
        iframeContent.style.height = "100%"; // Asegura que el iframe ocupe el alto de mainContent
        iframeContent.style.marginRight = "220px"; // Elimina el margen izquierdo del iframe
      }
    }
  }
  ...
  toggleMenu.addEventListener("click", function () {
    ...
    adjustMainContent(); // Ajusta dinámicamente el contenido principal
  });
