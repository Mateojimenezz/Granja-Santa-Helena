...
// Filtrar filas al hacer clic en el botón
document.getElementById('boton-buscar').addEventListener('click', function () {
  const filtro = document.getElementById('busqueda').value.toLowerCase(); // Texto ingresado
  const filas = document.querySelectorAll('#fechaEstadoTable tbody tr'); // Filas de la tabla

  filas.forEach(fila => {
    const textoFila = fila.textContent.toLowerCase(); // Texto de cada fila en minúsculas
    fila.style.display = textoFila.includes(filtro) ? '' : 'none'; // Mostrar/ocultar filas
  });
});

// Filtrar al escribir directamente (opcional)
document.getElementById('busqueda').addEventListener('input', function (event) {
  const filtro = event.target.value.toLowerCase(); // Texto ingresado
  const filas = document.querySelectorAll('#fechaEstadoTable tbody tr'); // Filas de la tabla
