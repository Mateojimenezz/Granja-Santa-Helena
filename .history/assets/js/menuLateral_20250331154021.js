  ...
  if (isLargeScreen()) {
    toggleMenu.addEventListener("click", function () {
      if (sidebar.style.display === "none" || sidebar.style.display === "") {
        ...
        mainContent.style.marginLeft = "220px"; // Restaura el contenido al margen original
      }
    });
...
const menuItems = document.querySelectorAll('.nav-item'); // Selecciona los ítems del menú superior

menuItems.forEach(item => {
  const submenu = item.querySelector('.submenu');
  if (submenu) {
    // Mostrar/ocultar el submenú al pasar el mouse (solo para pantallas grandes)
