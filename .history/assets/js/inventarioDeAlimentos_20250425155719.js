  ...
  document.getElementById('exportExcelButton').addEventListener('click', function (event) {
    ...
    rows.forEach(row => {
      const rowData = Array.from(row.cells).map(cell => {
        const input = cell.querySelector('input, select'); // Buscar input o select
    
        if (input) {
          // Si el input es un checkbox, devuelve "on" o "off"
          if (input.type === 'checkbox') {
            return input.checked ? 'on' : 'off';
          }
    
          // Si el input es un select, verifica que la opción seleccionada existe
        ...
        // Si no hay input, devolver el contenido de la celda
