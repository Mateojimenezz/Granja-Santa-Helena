...
document.addEventListener('change', function (event) {
   ...
   newRow.innerHTML = `
    ...
    <td>
     ...
     <td>
      ...
      <td>
        <span class="estado-alimento">Dismponible</span>
       </td>
 ...
 deleteUserButton.addEventListener('click', function () {
  ...
  function exportarExcel() {
    const tabla = document.getElementById('userTable'); // Seleccionar la tabla original
    const filas = tabla.querySelectorAll('tbody tr'); // Filas del cuerpo de la tabla
  
    // Crear una tabla temporal para procesar los valores dinámicos
    const tablaTemporal = tabla.cloneNode(true);
  
    // Procesar filas y celdas dinámicas
    tablaTemporal.querySelectorAll('tbody tr').forEach(fila => {
      const columnas = fila.querySelectorAll('td');
      columnas.forEach(celda => {
        const select = celda.querySelector('select'); // Detectar <select>
        const input = celda.querySelector('input'); // Detectar <input> (incluye checkbox y date)
  
        if (select) {
          // Capturar el texto de la opción seleccionada del <select>
          const textoSeleccionado = select.options[select.selectedIndex]?.text || 'Sin Seleccionar';
          celda.textContent = textoSeleccionado; // Reemplazar el <select> con su valor seleccionado
        } else if (input) {
          if (input.type === 'checkbox') {
            // Capturar estado del checkbox ("on" si está marcado, "off" si no lo está)
            celda.textContent = input.checked ? 'on' : 'off';
          } else if (input.type === 'date') {
            // Capturar valor de inputs tipo date
            celda.textContent = input.value; // Reemplazar el input con su valor
          } else {
            // Capturar valores de otros <input> (como texto o números)
    ...
    // Convertir la tabla procesada a un libro Excel
    const wb = XLSX.utils.table_to_book(tablaTemporal);
    const wbout = XLSX.write(wb, { bookType: 'xlsx', type: 'binary' });
  
    // Crear y descargar el archivo Excel
    const archivo = new Blob([s2ab(wbout)], { type: 'application/octet-stream' });
    const link = document.createElement('a');
    link.href = URL.createObjectURL(archivo);
    link.download = 'Inventario.xlsx'; // Nombre del archivo exportado
    link.click();
  }
  
  // Función auxiliar para manejar datos binarios
  function s2ab(s) {
