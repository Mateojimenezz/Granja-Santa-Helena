  ...
  function exportarExcel() {
    ...
    tablaTemporal.querySelectorAll('tbody tr').forEach(fila => {
      ...
      columnas.forEach(celda => {
        ...
        if (select) {
          const textoSeleccionado = select.options[select.selectedIndex]?.text.trim(); // Capturar el texto seleccionado
          celda.textContent = textoSeleccionado || 'Sin Seleccionar'; // Actualizar celda con el texto seleccionado
        
        } else if (input) {
          if (input.type === 'checkbox') {
            celda.textContent = input.checked ? 'on' : 'off';
          } else if (input.type === 'date') {
            const fecha = new Date(input.value + 'T00:00:00'); // Ajustar zona horaria
            const fechaLocal = fecha.toLocaleDateString('en-CA'); // Formato ISO YYYY-MM-DD
            celda.textContent = fechaLocal; // Actualizar celda con fecha local
          }
