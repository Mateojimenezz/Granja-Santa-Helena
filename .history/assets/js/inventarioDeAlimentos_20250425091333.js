...
document.addEventListener('change', function (event) {
 ...
 deleteUserButton.addEventListener('click', function () {
  ...
  function exportarExcel() {
    const tabla = document.getElementById('userTable'); // Seleccionar la tabla
    const filas = tabla.querySelectorAll('tbody tr'); // Filas del cuerpo de la tabla
  
    // Crear una copia temporal de la tabla para ajustar los valores dinámicos
    ...
    tablaTemporal.querySelectorAll('tbody tr').forEach((fila, index) => {
      ...
      columnas.forEach(celda => {
        const select = celda.querySelector('select'); // Verificar si hay un <select>
        if (select) {
          const textoSeleccionado = select.options[select.selectedIndex]?.text; // Capturar el texto de la opción seleccionada
          celda.innerHTML = textoSeleccionado; // Reemplazar el <select> con su valor seleccionado
        }
      });
    });
  
    // Convertir la tabla ajustada a un libro de Excel
