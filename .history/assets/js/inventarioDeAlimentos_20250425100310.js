  ...
  function exportarExcel() {
    ...
    tablaTemporal.querySelectorAll('tbody tr').forEach(fila => {
      ...
      columnas.forEach(celda => {
        const select = celda.querySelector('select'); // Detectar <select> en la celda
        const input = celda.querySelector('input'); // Detectar <input> (checkbox, texto o fecha)
        ...
        if (select) {
          ...
          celda.textContent = textoSeleccionado; // Reemplazar el <select> con su opción seleccionada
        } else if (input) {
          if (input.type === 'checkbox') {
            // Capturar estado del checkbox ("on" o "off")
            ...
            // Capturar valores de otros <input>
            celda.textContent = input.value || 'N/A'; // Reemplazar input con su valor
          }
        }
      });
    });
