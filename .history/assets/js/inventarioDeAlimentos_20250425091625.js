...
document.addEventListener('change', function (event) {
 ...
 deleteUserButton.addEventListener('click', function () {
  ...
  function exportarExcel() {
    ...
    const filas = tabla.querySelectorAll('tbody tr'); // Seleccionar filas del cuerpo de la tabla
  
    // Crear una tabla temporal para procesar los valores dinámicos
    const tablaTemporal = tabla.cloneNode(true);
  
    tablaTemporal.querySelectorAll('tbody tr').forEach((fila, index) => {
      const columnas = fila.querySelectorAll('td');
      columnas.forEach(celda => {
        const select = celda.querySelector('select'); // Verificar si hay un <select> en la celda
        const input = celda.querySelector('input'); // Verificar si hay un <input> en la celda
  
        if (select) {
          // Capturar el texto de la opción seleccionada
          const textoSeleccionado = select.options[select.selectedIndex]?.text;
          celda.textContent = textoSeleccionado; // Reemplazar <select> con su opción seleccionada
        } else if (input && input.type === 'date') {
          // Capturar el valor de la fecha en los inputs de tipo "date"
          celda.textContent = input.value; // Reemplazar el input con su valor
        } else if (input) {
          // Capturar otros valores de <input>
          celda.textContent = input.value; // Reemplazar input con su valor
        }
      });
    });
  
    // Convertir la tabla procesada a libro de Excel
    ...
    link.download = 'Inventario.xlsx'; // Nombre del archivo generado
    link.click();
  }
