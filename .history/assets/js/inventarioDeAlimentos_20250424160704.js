...
document.getElementById('busqueda').addEventListener('input', function (event) {
  const filtro = event.target.value.toLowerCase(); // Convertir el texto ingresado a minúsculas
  const filas = document.querySelectorAll('#fechaEstadoTable tbody tr'); // Seleccionar filas de la tabla

  filas.forEach(fila => {
    const textoFila = fila.textContent.toLowerCase(); // Convertir el texto de la fila a minúsculas
    fila.style.display = textoFila.includes(filtro) ? '' : 'none'; // Mostrar u ocultar la fila según el filtro
  });
});
