...
document.addEventListener('change', function (event) {
 ...
 deleteUserButton.addEventListener('click', function () {
  ...
  function exportarExcel() {
    ...
    tablaTemporal.querySelectorAll('tbody tr').forEach(fila => {
      ...
      columnas.forEach(celda => {
        const select = celda.querySelector('select'); // Detectar <select> usando su id o class
        const input = celda.querySelector('input'); // Detectar <input> (checkbox, texto o fecha)
  
        if (select) {
          const textoSeleccionado = select.options[select.selectedIndex]?.text.trim(); // Capturar texto seleccionado
          celda.textContent = textoSeleccionado || 'Sin Seleccionar';
        } else if (input) {
          if (input.type === 'checkbox') {
            celda.textContent = input.checked ? 'on' : 'off'; // Capturar estado del checkbox
          } else if (input.type === 'fecha-caducidad') {
            celda.textContent = input.value; // Capturar la fecha directamente sin modificarla
          } else {
            celda.textContent = input.value || 'N/A'; // Capturar valor de otros inputs
          }
        }
      });
    });
  
    const wb = XLSX.utils.table_to_book(tablaTemporal); // Convertir la tabla a libro de Excel
  ...
  // Asignar el evento de exportación al botón
