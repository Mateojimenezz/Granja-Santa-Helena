...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const mainContent = document.querySelector("main"); // Contenedor principal donde se carga el contenido
  ...
  function configureSidebarMenu() {
    ...
    menuItems.forEach((item, index) => {
      ...
      // **Funcionalidad para cargar páginas dinámicamente**
      clonedLink.addEventListener("click", function (e) {
        e.preventDefault();

        const page = clonedLink.getAttribute("data-page"); // Obtén la ruta desde data-page
        if (page) {
          fetch(page)
            .then(response => {
            ...
            .then(html => {
              mainContent.innerHTML = html; // Reemplaza el contenido en <main>
            })
            .catch(error => {
              console.error("Error al cargar el contenido dinámico:", error);
              mainContent.innerHTML = "<p>Error al cargar el contenido. Intenta nuevamente.</p>";
            });
        }
      });
  ...
  toggleMenu.addEventListener("click", function () {
    if (sidebar.style.display === "none" || sidebar.style.display === "") {
      ...
      if (isLargeScreen()) {
        ...
        mainContent.style.maxWidth = `calc(100% - ${sidebar.offsetWidth}px)`;
      }
    } else {
