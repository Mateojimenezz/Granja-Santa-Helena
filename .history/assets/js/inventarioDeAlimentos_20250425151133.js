...
document.addEventListener('change', function (event) {
 ...
 deleteUserButton.addEventListener('click', function () {
  ...
  function exportarExcel() {
    const tabla = document.getElementById('userTable'); // Seleccionar tabla
    ...
    tablaTemporal.querySelectorAll('tbody tr').forEach(fila => {
      ...
      columnas.forEach(celda => {
        ...
        if (select) {
          const textoSeleccionado = select.options[select.selectedIndex]?.text.trim(); // Capturar texto de la opción seleccionada
          console.log('Texto seleccionado:', textoSeleccionado); // Ver qué valor se está capturando
          celda.textContent = textoSeleccionado || 'Sin Seleccionar'; // Actualizar la celda con el texto seleccionado
        }
         else if (input) {
          if (input.type === 'checkbox') {
            celda.textContent = input.checked ? 'on' : 'off';
          } else if (input.type === 'date') {
            const fecha = input.value; // Capturar la fecha directamente sin transformarla
