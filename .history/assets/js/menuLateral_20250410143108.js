  ...
  closeMenu.addEventListener("click", function () {
   ...
   menuItems.forEach(item => {
    item.addEventListener("click", function (e) {
      ...
      const page = this.getAttribute("data-page"); // Ruta de la página a cargar
      fetch(page)
        .then(response => {
        ...
        .then(html => {
          mainContent.innerHTML = html; // Reemplaza el contenido de <main>
          if (window.innerWidth < 768) {
            sidebar.style.display = "none"; // Oculta el menú en pantallas pequeñas
          }
        })
        .catch(error => {
          console.error("Error al cargar el contenido:", error);
          mainContent.innerHTML = "<p>No se pudo cargar el contenido. Intenta nuevamente.</p>";
        });
    });
   }
  );
