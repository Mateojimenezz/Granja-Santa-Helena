...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function configureSidebarMenu() {
    ...
    menuItems.forEach((item, index) => {
      ...
      // **Evento para cargar contenido dinámico**
      clonedLink.addEventListener("click", function (e) {
        e.preventDefault();
        
        // Aquí puedes definir qué página cargar según el ítem
        const pages = [
          "Gestion_de_permiso_roles.html",     // Página para el primer ítem (Usuarios)
          "usuarios.html",   // Página para el segundo ítem (Inventarios)
          "inventarios.html",// Página para el tercer ítem (Registro de Producción)
          "produccion.html", // Página para el cuarto ítem (Salud Animal)
          "ventas.html",     // Página para el quinto ítem (Ventas)
        ];
        
        fetch(pages[index]) // Carga la página correspondiente
          .then(response => response.text())
          .then(html => {
            mainContent.innerHTML = html; // Actualiza el contenido principal
            configureSidebarMenu(); // Configura el menú lateral nuevamente
            
          })
          .catch(error => {
            console.error("Error al cargar la página:", error);
            mainContent.innerHTML = `<p>Error al cargar la página. Inténtalo de nuevo.</p>`;
          });
      });
