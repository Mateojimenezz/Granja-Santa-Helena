...
function filtrarTabla() {
  const filtro = document.getElementById('busqueda').value.toLowerCase(); // Texto ingresado a minúsculas
  const filas = document.querySelectorAll('#miTabla tbody tr'); // Filas de la tabla

  filas.forEach(fila => {
    const textoFila = fila.textContent.toLowerCase(); // Texto de cada fila a minúsculas
    fila.style.display = textoFila.includes(filtro) ? '' : 'none'; // Mostrar u ocultar filas
  });
}
...
// Evento para el input (filtrado al escribir)
document.getElementById('busqueda').addEventListener('input', filtrarTabla);


// Evento al escribir directamente en la barra
document.getElementById('busqueda').addEventListener('input', filtrarTabla);
