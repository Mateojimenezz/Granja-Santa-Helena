...
// Asegúrate de inicializar correctamente `doc`
const doc = new jsPDF();
...
doc.autoTable({
    head: [['Seleccionar', 'Fecha', 'Lote', 'Categoría', 'Tipo de Alimento', 'Cantidad de Salida', 'Destino', 'Responsable']], // Encabezados de la tabla
    body: rows.slice(1).map(row => { // Ignorar la primera fila si esta no es parte de los datos
      return Array.from(row.cells).map(cell => {
        const input = cell.querySelector('input, select');
        return input ? (input.value || input.options[input.selectedIndex]?.text) : cell.textContent.trim();
      });
    }),
    theme: 'grid', // Opcional: define un tema de tabla
    startY: 20 // Define el punto inicial Y para la tabla (evita superposiciones con otros elementos)
});

// Guardar como archivo PDF con el nombre deseado
doc.save('Registro_de_Alimentos_Diario.pdf'); // Asegúrate de que el nombre sea único y esté escrito correctamente
